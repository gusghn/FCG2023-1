...
void save(string fileName)
{
	ofstream arq1(fileName, std::ios::out | std::ios::binary);
	
	if (arq1.is_open()) 
	{		
		arq1.write(reinterpret_cast<char*>(&voxelmap), sizeof(voxelmap));
		arq1.close();
	}
...
void open(string fileName)
{
	ifstream arq1(fileName, std::ios::in | std::ios::binary);

	if (arq1.is_open())
	{
		arq1.seekg(0);
		arq1.read((char*)voxelmap, sizeof(voxelmap));
		arq1.close();
	}	
}
